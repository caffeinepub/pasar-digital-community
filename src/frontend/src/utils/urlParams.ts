...
/**
 * Captures an invite token from the URL and persists it to sessionStorage
 * Then removes it from the visible URL without reloading the page
 * This allows the token to survive navigation and Internet Identity sign-in flow
 *
 * @param paramName - The name of the invite token parameter (default: 'inviteToken')
 * @returns The captured token if found, null otherwise
 */
export function captureAndPersistInviteToken(paramName: string = 'inviteToken'): string | null {
  // Check if already in session
  const existingToken = getSessionParameter(paramName);
  if (existingToken !== null) {
    return existingToken;
  }

  // Try to extract from URL
  const urlToken = getUrlParameter(paramName);
  if (urlToken !== null) {
    // Store in session for persistence
    storeSessionParameter(paramName, urlToken);
    // Remove from visible URL
    clearParamFromHash(paramName);
    return urlToken;
  }
...
/**
 * Gets the persisted invite token from sessionStorage
 * Used by onboarding page to retrieve the token after sign-in
 *
 * @param paramName - The name of the invite token parameter (default: 'inviteToken')
 * @returns The persisted token if found, null otherwise
 */
export function getPersistedInviteToken(paramName: string = 'inviteToken'): string | null {
 ...
 * @param paramName - The name of the invite token parameter (default: 'inviteToken')
 */
export function clearPersistedInviteToken(paramName: string = 'inviteToken'): void {
